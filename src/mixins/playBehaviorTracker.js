...
export default {
  ...
  methods: {
    handleTrackChange(newTrack, oldTrack) {
      ...
      const isSkip = playedDuration < this.skipThreshold && 
                     !this.isLikedTrack(oldTrack.id);
      
      if (isSkip) {
        this.recordSkipEvent(oldTrack.id, playedDuration);
      } else if (playedDuration > this.skipThreshold) {
        // Record as completed play
        const completed = playedDuration >= (this.previousTrackDuration || 0) * 0.8;
    ...
    recordPlayEvent(songId, duration, completed = false) {
      ...
      recordPlay(this.userId, songId, Math.floor(duration), completed)
        .catch(err => console.error('Failed to record play:', err));
    },
    
    recordSkipEvent(songId, skipTime) {
      if (!this.userId || !songId) return;
      
      recordSkip(this.userId, songId, Math.floor(skipTime))
